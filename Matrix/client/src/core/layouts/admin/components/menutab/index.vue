...
<template>
  <div class="custom-tabs border" :class="{ 'dark-mode': isDark }">
    ...
    <div class="tabs-header-wrapper">
      <!-- 左滚动按钮 -->
      <div v-show="showLeftButton" class="scroll-button left" :class="{ pressing: isPressingLeft }"
        @mousedown="() => startPress('left')" @mouseup="stopPress" @mouseleave="stopPress" @click="scrollLeft">
        <el-icon>
          <ArrowLeft />
        </el-icon>
      </div>
      ...
      <div class="tabs-header" ref="tabsHeader" @wheel.prevent="handleWheel">
        <div class="tabs-nav-wrap" ref="tabsNav" :style="{ transform: `translateX(${-scrollOffset}px)` }">
          <div v-for="(tab, index) in tabs" :key="tab.path" class="tab-item" :class="{
            active: tab.path === activeTab,
            dragging: draggingIndex === index,
            'drag-over': dragOverIndex === index,
            'fixed-tab': pathFilter.includes(tab.path)
          }" :draggable="!pathFilter.includes(tab.path)" @dragstart="handleDragStart($event, index)"
            @dragend="handleDragEnd" @dragover="handleDragOver($event, index)" @drop="handleDrop($event, index)"
            @click="switchTab(tab.path)" @contextmenu="handleContextMenu($event, tab)">
            <el-icon v-if="tab.icon">
              <!-- <component :is="icons[tab.icon]" /> -->
            </el-icon>
            <span class="tab-label">{{ truncateText(tab.name, 128) }}</span>
            <el-icon v-if="!pathFilter.includes(tab.path)" class="close-icon" @click.stop="closeTab(tab.path)">
              <Close />
            </el-icon>
          </div>
        </div>
      ...
      <!-- 右滚动按钮 -->
      <div v-show="showRightButton" class="scroll-button right" :class="{ pressing: isPressingRight }"
        @mousedown="() => startPress('right')" @mouseup="stopPress" @mouseleave="stopPress" @click="scrollRight">
        <el-icon>
          <ArrowRight />
        </el-icon>
      </div>
    ...
    <!-- 右键菜单 -->
    <div v-show="showContextMenu" class="context-menu" :style="{
      left: contextMenuPosition.x + 'px',
      top: contextMenuPosition.y + 'px'
    }">
      <div class="context-menu-item" @click="refreshCurrentTab">刷新当前</div>
      <div v-if="contextMenuTab && !pathFilter.includes(contextMenuTab.path)" class="context-menu-item"
